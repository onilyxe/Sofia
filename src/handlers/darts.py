...
@games_router.message(Command(Games.DARTS), IsChat(), CooldownFilter(Games.DARTS, True))
...
async def darts_callback_bet_play(callback: types.CallbackQuery, callback_data: DartsCallback, db: Database, chat_user):
    ...
    await callback.message.edit_text(Text("🎯 Прицілюємося..").as_markdown())
    ...
    try:
        await callback.bot.answer_callback_query(callback.id, "🎯 Дротик кинуто")
